<!-- app/components/form-builder/properties/NumberFieldProperties.vue -->
<template>
    <div>
        <!-- PROPERTIES Section -->
        ...
        <CollapsibleSection title="NUMBER SETTINGS" :default-open="true">
            <!-- Min/Max Row -->
            <div class="grid grid-cols-2 gap-4">
                <!-- Min -->
                <BaseInput :model-value="localField.min"
                    @update:model-value="updateField('min', $event !== '' ? parseFloat($event) : null)" label="Min Value"
                    type="number" placeholder="No min" />

                <!-- Max -->
                <BaseInput :model-value="localField.max"
                    @update:model-value="updateField('max', $event !== '' ? parseFloat($event) : null)" label="Max Value"
                    type="number" placeholder="No max" />
            </div>

            <!-- Allow Decimal Switch -->
            <ToggleSwitch :model-value="localField.allowDecimal"
                @update:model-value="updateField('allowDecimal', $event)" label="Allow Decimals"
                subtext="Enable decimal/floating point numbers" />
        </CollapsibleSection>
