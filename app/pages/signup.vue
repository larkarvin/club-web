        ...
        <div class="flex flex-col flex-1 lg:w-1/2 w-full">
          ...
          <div class="flex flex-col justify-center flex-1 w-full max-w-md mx-auto">
            ...
            <div>
              ...
              <Form @submit="handleSubmit" :validation-schema="schema" v-slot="{ isSubmitting, setErrors }">
                <div class="space-y-5">
                  <FormInput name="club_name" label="Club Name" placeholder="Enter your club name" required />

                  <FormInput name="name" label="Your Name" placeholder="Enter your name" required />

                  <FormInput name="email" type="email" label="Email" placeholder="Enter your email" required />

                  <FormPasswordInput name="password" label="Password" placeholder="Enter your password" required />

                  <FormPasswordInput name="password_confirmation" label="Confirm Password"
                    placeholder="Confirm your password" required :show-strength="false" />
...
import FormPasswordInput from '@/components/forms/FormPasswordInput.vue';
import { schemas } from '@/utils/validationSchemas';
import { Form } from 'vee-validate';

useHead({
  title: 'Sign Up a New Club'
});

const router = useRouter();
const { register, isAuthenticated } = useAuth();
const { redirectToClub } = useUrlHelper();
...
const handleSubmit = async (values: any, { setErrors }: any) => {
  ...
  if (result.success && result.data) {
    // Get the club subdomain from response
    const clubSubdomain = result.data.club?.subdomain;

    if (clubSubdomain) {
      // Redirect to club subdomain dashboard
      console.log('Redirecting to club:', clubSubdomain);
      redirectToClub(clubSubdomain, '/dashboard');
    } else {
      // Fallback: redirect to signin if no club subdomain
      console.warn('No club subdomain in response, redirecting to signin');
      await router.push('/auth/signin?registered=true');
    }
  } else if (result.errors) {
    setErrors(result.errors); // Map Laravel errors to form
  }
};
