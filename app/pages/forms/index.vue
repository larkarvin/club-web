...
<template>
    <admin-layout>
        <page-breadcrumb pageTitle="Forms" />

        <div class="rounded-2xl border border-gray-200 bg-white pt-4 dark:border-gray-800 dark:bg-white/[0.03]">
            <div class="flex flex-col gap-2 px-5 mb-4 sm:flex-row sm:items-center sm:justify-between sm:px-6">
                <div>
                    <h3 class="text-lg font-semibold text-gray-800 dark:text-white/90">Club Forms</h3>
                </div>
                <div class="flex flex-col gap-3 sm:flex-row sm:items-center">
                    ...
                    <form @submit.prevent>
                        <div class="relative">
                            ...
                            <input v-model="searchQuery" type="text" placeholder="Search forms..."
            ...
            <!-- Empty State -->
            <div v-else-if="forms.length === 0" class="flex flex-col items-center justify-center py-20 px-4">
                ...
                <h3 class="text-lg font-medium text-gray-800 dark:text-white mb-2">No forms yet</h3>
                <p class="text-gray-500 dark:text-gray-400 mb-4 text-center">Create your first form to start collecting
                    data from your members.</p>
                <NuxtLink to="/forms/new"
                    class="inline-flex items-center justify-center gap-2 rounded-lg bg-brand-500 px-4 py-2.5 text-sm font-medium text-white shadow-theme-xs hover:bg-brand-600">
                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                ...
                <div class="max-w-full px-5 overflow-x-auto">
                    <table class="min-w-full">
                        ...
                        <tbody class="divide-y divide-gray-200 dark:divide-gray-700">
                            <tr v-for="form in paginatedForms" :key="form.id" class="hover:bg-gray-50 dark:hover:bg-white/[0.02]">
                                <td class="py-4 px-4 whitespace-nowrap">
                                    <div class="flex items-center">
                                        <div
                                            class="shrink-0 w-10 h-10 rounded-lg bg-brand-100 dark:bg-brand-900/30 flex items-center justify-center">
                                ...
                                <td class="px-4 py-4 whitespace-nowrap">
                                    <div class="text-sm text-gray-500 dark:text-gray-400">
                                        {{ form.processing_fee ? `$${form.processing_fee}` : 'Free' }}
                                    </div>
                                </td>
                                <td class="px-4 py-4 whitespace-nowrap">
                                    <span :class="[
                                    ...
                                    <div class="flex justify-center">
                                        <TableDropdown>
                                            ...
                                            <template v-slot:dropdown-content>
                                                <NuxtLink :to="`/forms/${form.id}/fields`"
                                                    class="flex w-full px-3 py-2 font-medium text-left text-gray-500 rounded-lg text-theme-xs hover:bg-gray-100 hover:text-gray-700 dark:text-gray-400 dark:hover:bg-white/5 dark:hover:text-gray-300">
                                                    Edit Fields
                                                </NuxtLink>
                                                <button @click="toggleFormStatus(form)"
                                                    class="flex w-full px-3 py-2 font-medium text-left text-gray-500 rounded-lg text-theme-xs hover:bg-gray-100 hover:text-gray-700 dark:text-gray-400 dark:hover:bg-white/5 dark:hover:text-gray-300">
                                                    {{ form.enabled ? 'Disable' : 'Enable' }}
                                                </button>
                                                <button @click="confirmDelete(form)"
                                                    class="flex w-full px-3 py-2 font-medium text-left text-error-500 rounded-lg text-theme-xs hover:bg-error-50 hover:text-error-600 dark:hover:bg-error-500/10">
                                                    Delete
                                                </button>
                                            </template>
                                        </TableDropdown>
        ...
        <div v-if="formToDelete" class="fixed inset-0 z-99999 flex items-center justify-center bg-black/50"
            @click.self="formToDelete = null">
            <div class="bg-white dark:bg-gray-900 rounded-lg shadow-xl w-full max-w-md p-6">
                <h3 class="text-lg font-semibold text-gray-800 dark:text-white mb-2">Delete Form</h3>
                <p class="text-gray-600 dark:text-gray-400 mb-6">
                    Are you sure you want to delete "{{ formToDelete.title }}"? This action cannot be undone.
                </p>
                <div class="flex justify-end gap-3">
                    <button @click="formToDelete = null"
                    ...
                    <button @click="deleteForm" :disabled="isDeleting"
                        class="px-4 py-2 text-sm font-medium text-white bg-error-500 rounded-lg hover:bg-error-600 disabled:opacity-50">
...
const toggleFormStatus = async (form) => {
    try {
        await formsApi.updateSettings(form.id, { enabled: !form.enabled })
        form.enabled = !form.enabled
        toast.success(`Form ${form.enabled ? 'enabled' : 'disabled'}`)
    } catch (error) {
...
const deleteForm = async () => {
    ...
    try {
        await formsApi.delete(formToDelete.value.id)
